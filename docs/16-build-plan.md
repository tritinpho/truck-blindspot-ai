...
- [x] **Day/night/auto HMI theme** (cosmetic polish) — high-contrast daylight + dark night mode,
  ...
  unchanged across themes. tsc/build/26 tests green; both themes visually verified.
...
- Own the eval-report skeleton; enforce **L4-bench** (not L3-sim) headline metrics
  ...
  Sim inputs + the honest met-in-sim/NEEDS-L4 split are assembled in
  [`21-evaluation-report-inputs.md`](21-evaluation-report-inputs.md) (regenerated by
  [`tools/eval_report.py`](../tools/eval_report.py)).

**Firmware / W (student · best-effort · non-gating).**
- **Build to [`20-firmware-contract-checklist.md`](20-firmware-contract-checklist.md)** — the
  self-contained spec + conformance checklist (every field, group-fire, clocks, heartbeat/LWT,
  bring-up acceptance test) + the offline [`tools/validate_message.py`](../tools/validate_message.py)
  self-check, so a student builds to it without reading the rest of the repo.
